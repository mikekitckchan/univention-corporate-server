...
if __name__ == '__main__':
	...
	parser.add_option("--write2ucs", action="store_true", help="Write MS GPO settings from Samba 4 to UCS", default=False)
	parser.add_option("--write2samba4", action="store_true", help="Write MS GPO settings from UCS to Samba 4", default=False)
	parser.add_option("--only-override-empty", action="store_true", help="The parameter controls that the attribute is only overwritten in case it is empty. This can only be used in write2ucs mode.", default=False)
	parser.add_option("--binddn", action="store", help="Binddn for UCS LDAP connection")
	parser.add_option("--bindpwd", action="store", help="Password for UCS LDAP connection")
	parser.add_option("--bindpwdfile", action="store", help="Password file for UCS LDAP connection")
