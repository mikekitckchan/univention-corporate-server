...
#
# Check if a service exists and is not masked
#   This function can be used if "systemctl" failures due to non existing
#   or masked services should be avoided.
#
service_can_be_controlled() {
	load_state=$(systemctl show --type=service --property LoadState --value "$1")
	...
	source_path=$(systemctl show --type=service --property SourcePath --value "$1")
	if [ -n "$source_path" ]; then
		init_path="/etc/init.d/${1%.service}"
		if [ "$source_path" = "$init_path" ] && [ ! -x "$init_path" ]; then
			# Service generated by systemd-sysv-generator from init script
			# and init script is masked by removing the executable bit
			return 2
		fi
	fi
	return 0
